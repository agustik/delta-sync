(() => {
  ...
  document.getElementById("filepicker").addEventListener("change", async (event) => {
    ...
    for (const file of event.target.files) {
      ...
      const upload = await new DeltaSync(file, {
        url: `/api/.delta-sync/${file.name}?foo=bar&d=${/* @__PURE__ */ new Date()}`,
        updates: (event2, data) => {
          console.log("Got update event", event2, data);
  ...
  var socket = new WebSocket("ws://localhost:9000/api/.delta-sync/blarg");
})();
